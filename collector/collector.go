...
func Run(ctx context.Context, logger Logger, plugin sink.Interface, options RunOptions) error {
	...
	g.Go(func() error {
		defer logger.Info("Reader loop exited")
		...
		for {
			...
			if err != nil {
				...
				if ctx.Err() != nil {
					logger.Info("Context cancelled during read")
					return ctx.Err()
				}
			...
			if err := binary.Read(bytes.NewBuffer(record.RawSample), binary.LittleEndian, &event); err != nil {
				logger.Error("Failed to decode event", slog.Any("err", err))
				continue
			}
	...
	g.Go(func() error {
		<-ctx.Done() // Wait for cancellation (client disconnect, shutdown, etc.)
		logger.Info("Context cancelled, closing reader")
		return reader.Close() // Will unblock the Read() above
	})
