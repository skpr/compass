...
var (
	red     = gchalk.WithHex(color.Red)
	orange  = gchalk.WithHex(color.Orange)
	yellow  = gchalk.WithHex(color.Yellow)
	dimBlue = gchalk.WithHex(color.DimBlue)
	dimGrey = gchalk.WithHex(color.DimGrey)
)
...
func (c *Component) Render(span Span) string {
	...
	delimiter := dimGrey.Paint("｜")
	return fmt.Sprintf("%s%s%s%s %dms",
		delimiter,
		strings.Repeat(" ", pre),
		colorForFill(FractionDuration(span.Duration, c.Duration), strings.Repeat(Block, fill)),
		strings.Repeat(" ", post),
